...
# PowerShell installer script for UniGoDesktop (install.ps1)
# Compatible with Windows PowerShell and PowerShell Core
# Usage:
#   Invoke-WebRequest -Uri https://raw.githubusercontent.com/snowdreamtech/UniGoDesktop/main/install.ps1 -OutFile install.ps1; .\install.ps1 --version v0.7.0
...
function Show-Help {
    ...
    Write-Info "  --install-dir   Directory to place the binary (default: `$HOME\bin for normal user, C:\Program Files\UniGoDesktop for admin)"
...
if (-not $InstallDir) {
    ...
    if ($isAdmin) {
        $InstallDir = "C:\Program Files\UniGoDesktop"
    } else {
        $InstallDir = "$HOME\bin"
    }
}
...
if (Test-Path $targetPath) {
    try {
        ...
        $installedVer = $verOutput -split "`n" | Where-Object { $_ -match "^unigodesktop version" } | Select-Object -First 1
