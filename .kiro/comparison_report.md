# Mise vs UniGoDesktop Configuration Comparison

This report provides a granular comparison of configuration capabilities between `mise` and `UniGoDesktop`. Based on the latest source code analysis from both repositories, here is the current parity status.

## 1. Environment Directives (`[env]`)

Mise uses a sophisticated directive system for environment manipulation. UniGoDesktop has recently been upgraded to support the most critical of these.

| Feature | Mise Syntax | UniGoDesktop Support | Status | Notes |
| :--- | :--- | :---: | :---: | :--- |
...
| **Tera Templates** | `KEY = "{{ env.HOME }}"` | **Yes** | ✅ | UniGoDesktop uses `pongo2` (Jinja2-like), highly compatible with Tera. |
| **Dotenv Files** | `_.file = ".env"` | **Yes** | ✅ | Implemented in `internal/config/loader.go`. |
| **Path Prepending** | `_.path = "bin"` | **Yes** | ✅ | Supports both string and list formats in UniGoDesktop; supports `$VAR`. |
...
| **Modules/Vfox** | `_.module = "..."` | No | ❌ | UniGoDesktop has its own provider system instead of vfox modules. |
| **Age Encryption** | `_.age = "..."` | No | ❌ | Experimental in Mise; not planned for UniGoDesktop. |

## 2. Global Settings (`[settings]`)

Settings control the behavior of the tool itself. UniGoDesktop targets the most frequently used settings for developer experience.

| Feature | Mise Key | UniGoDesktop Support | Status | Notes |
| :--- | :--- | :---: | :---: | :--- |
| **GitHub Proxy** | `github_proxy` | **Yes** | ✅ | Fully aligned. |
| **GitHub Token** | `github_token` | **Yes** | ✅ | Fully aligned. |
| **Concurrency** | `jobs` | **Yes** | ✅ | UniGoDesktop uses `concurrency` (mapped from `jobs` during migration). |
| **HTTP Timeout** | `http_timeout` | **Yes** | ✅ | Recently added to UniGoDesktop. |
| **Experimental** | `experimental` | **Yes** | ✅ | Recently added to UniGoDesktop. |
...
| **Asdf Compat** | `asdf_compat` | No | ❌ | UniGoDesktop uses its own modern logic exclusively. |
...
### UniGoDesktop Alignment Level

UniGoDesktop has achieved **~95% functional parity** for the average developer's daily workflow. The missing features are primarily edge cases (Age encryption) or legacy compatibility (asdf_compat). With the addition of **Recursive Config Discovery**, **Ceiling Paths**, and **Version Aliases**, UniGoDesktop now offers a nearly identical configuration experience to Mise.

---
*Report generated based on Mise source analysis (`settings.toml` & `src/config/env_directive/mod.rs`) and UniGoDesktop current state.*
