...
// install.js - Root package entry point for @snowdreamtech/unigodesktop
//
...
function getPlatformPackageSuffix() {
  ...
  throw new Error(
    `Unsupported platform: ${platform}-${arch}\n` +
      "Please open an issue at https://github.com/snowdreamtech/unigodesktop/issues"
  );
}
...
function resolveBinary(suffix) {
  const pkgName = `@snowdreamtech/unigodesktop-${suffix}`;
  const binaryName = process.platform === "win32" ? "unigodesktop.exe" : "unigodesktop";
  ...
  throw new Error(
    `Could not find binary for ${pkgName}.\n` +
      "Try reinstalling @snowdreamtech/unigodesktop:\n" +
      "  npm install @snowdreamtech/unigodesktop\n\n" +
...
if (isPostInstall) {
  ...
  try {
    ...
    if (result.status === 0) {
      process.stdout.write(`✅ unigodesktop installed successfully (${binPath})\n`);
    }
  } catch (err) {
    // Non-fatal: postinstall failure should not break npm install
    process.stderr.write(`⚠️  unigodesktop: ${err.message}\n`);
  }
} else {
